...
neighbors2 <- vector(length = 0)
neighbors3 <- vector(length = 0)
for (i in nonzero_bits){
  distances <- t( apply(an_seq[nonzero_bits,], 1, FUN =  hamming.distance, y = an_seq[i,]))
  n1 <- which(distances == 1, arr.ind = TRUE)
  neighbors <- c(neighbors,n1)
  n2 <- which(distances <= 2, arr.ind = TRUE)
  neighbors2 <- c(neighbors2, n2)
  n3 <- which(distances <= 3, arr.ind = TRUE)
  neighbors3 <- c(neighbors3, n3)
}
...
```{r}
numnonzeros_1 <- sum(sim1full_all_pf[,3] != 0)
numnonzeros_1

sum(sim1full_all_pf[,3] >= (1e-10))

nonzeroes1 <- which((sim1full_all_pf[,3]!= 0), arr.ind = TRUE)
nearnonzeroes1 <- which((sim1full_all_pf[,3]>= (1e-10)), arr.ind = TRUE)

falsenonzeros_distance <- vector(length = 0)
for(x in nonzeroes1){
  distances <- t( apply(an_seq[nonzero_bits,], 1, FUN =  hamming.distance, y = an_seq[x,]))
  falsenonzeros_distance <- c(falsenonzeros_distance, distances)
}

distances1 <- unique(falsenonzeros_distance)
distances1

library(dplyr)
summary(as.data.frame(falsenonzeros_distance))

nearnonzeroes_distance <- vector(length = 0)
for(x in nearnonzeroes1){
  distances <- t( apply(an_seq[nonzero_bits,], 1, FUN =  hamming.distance, y = an_seq[x,]))
  nearnonzeroes_distance <- c(nearnonzeroes_distance, distances)
}
neardistances1 <- unique(nearnonzeroes_distance)
neardistances1
summary(as.data.frame(nearnonzeroes_distance))

neighbors9 <- vector(length = 0)
for (i in nonzero_bits){
  distances <- t( apply(an_seq[nonzero_bits,], 1, FUN =  hamming.distance, y = an_seq[i,]))
  n1 <- which(distances <= 9, arr.ind = TRUE)
  neighbors9 <- c(neighbors9,n1)
}
n9 <- unique(neighbors9)
nonzeron9 <- sort(unique(c(nonzero_bits, n9)))
sum <- 0
for (x in nonzeron9){
  # sum <- sum + sim1full_all_pf[x,3]
  print(x)
}
sum
```
